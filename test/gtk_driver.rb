  ...
  class Session
    ...
    # a texture that knows how to write a PNG.
      def render(widget, path)
        # One renderer for the whole run: realising and unrealising a fresh
        # Gsk::CairoRenderer per shot leaves the next snapshot with no render
        # node at all, so only the first screenshot of a run comes out.
        @renderer ||= Gsk::CairoRenderer.new.tap { |renderer| renderer.realize(nil) }

        # A widget with a queued redraw snapshots to nothing, so let GTK settle
        # its pending frame before asking for the node.
        ...
        Gtk::Snapshot.new.then do |snapshot|
          ...
          snapshot.to_node.then do |node|
            if node.nil?
              raise "widget produced no render node (#{widget.class} #{widget.width}x#{widget.height} " \
                    "mapped=#{widget.mapped?} realized=#{widget.realized?} visible=#{widget.visible?})"
            end
      ...
      def report
        ...
        (@failed_checks + @errored_steps).length.then do |total|
          ...
          # exit! rather than exit: tearing an Adwaita widget tree down from
          # Ruby's finalizers segfaults in ruby-gnome 4.3.9, which would turn a
          # clean run into a spurious failure. See FINDINGS.md.
          $stdout.flush
          exit!(total.zero? ? 0 : 1)
        end
      end
